...
def earlyAlarm(alarmSet, fileName):
    try:
        document = Document()
        ...
        tableContent = tabP.add_run(alarmSet.getclassName() + "班")
        tableContent.font.size = Pt(12)
        tabP.alignment = WD_ALIGN_PARAGRAPH.CENTER
        ...
        tableContent = tabP.add_run(alarmSet.getstudentName())
        tableContent.font.size = Pt(12)
        tabP.alignment = WD_ALIGN_PARAGRAPH.CENTER
        ...
        tableContent = tabP.add_run(alarmSet.getstudentId())
        tableContent.font.size = Pt(12)
        tabP.alignment = WD_ALIGN_PARAGRAPH.CENTER
        ...
        notPassInfoNum = 0
        colElementNum = 0
        for curNotPassInfo in alarmSet.getnotPassInfo():
            tabP = table.cell(math.floor(notPassInfoNum / 2) + 1, colElementNum % 8).paragraphs[0]
            tableContent = tabP.add_run(curNotPassInfo.getsemester())
            tableContent.font.size = Pt(12)
            colElementNum += 1

            tabP = table.cell(math.floor(notPassInfoNum/2) + 1, colElementNum % 8).paragraphs[0]
            tableContent = tabP.add_run(curNotPassInfo.getcourseName())
            tableContent.font.size = Pt(12)
            colElementNum += 1

            tabP = table.cell(math.floor(notPassInfoNum/2) + 1, colElementNum % 8).paragraphs[0]
            tableContent = tabP.add_run(str(curNotPassInfo.getachievement()))
            tableContent.font.size = Pt(12)
            colElementNum += 1

            tabP = table.cell(math.floor(notPassInfoNum/2) + 1, colElementNum % 8).paragraphs[0]
            tableContent = tabP.add_run(str(curNotPassInfo.getcredit()))
            tableContent.font.size = Pt(12)
            colElementNum += 1
            notPassInfoNum += 1
        ...
        tableContent = tabP.add_run('该同学因不及格学分达到学业预警标准，特告知学生及家长，以作警示，如果学生以后再出现不及格现象，可能造成以下后果：\n')
        tableContent.font.size = Pt(12)
        tableContent = tabP.add_run('一、学生在校期间，累计不及格课程学分超过25学分，应予退学。\n')
        tableContent.font.size = Pt(12)
        tableContent = tabP.add_run('二、学生在学校基本学制年限内，未达到毕业要求的，按结业处理。\n')
        tableContent.font.size = Pt(12)
        tableContent = tabP.add_run('学生签名：\n')
        tableContent.font.size = Pt(12)
...
def currentAlarm(alarmSet, fileName):
    try:
        document = Document()
        document.styles['Normal'].font.name = u'宋体'
        document.styles['Normal']._element.rPr.rFonts.set(qn('w:eastAsia'), u'宋体')
        ...
        p = document.add_paragraph()
        paragraphHead = p.add_run('开封大学学业警告通知书')
        paragraphHead.bold = True
        paragraphHead.font.size = Pt(16)
        p.paragraph_format.alignment = WD_ALIGN_PARAGRAPH.CENTER
        ...
        tabP = table.cell(1, 0).paragraphs[0]
        tableContent = tabP.add_run('该同学因不及格学分超过20学分，特告知学生及家长，以作警告，希望学生认真总结教训并于一周内写出“学业改进计划及保证书”交院部审查，希望家长督促学生端正学习态度、及时改正问题，以使学生顺利完成学业。如果学生以后再出现不及格现象，可能造成以下后果：\n')
        tableContent.font.size = Pt(12)
        tableContent = tabP.add_run('一、学生在校期间，累计不及格课程学分超过25学分，应予退学。\n')
        tableContent.font.size = Pt(12)
        tableContent = tabP.add_run('二、学生在学校基本学制年限内，未达到毕业要求的，按结业处理。\n')
...
def generateAlarm(alarmSet, targetPath):
    try:
        targetFilePath = targetPath + "学业预警"
        targetFilePath2 = targetPath + "学业警告"
        if not os.path.exists(targetFilePath):
            os.makedirs(targetFilePath)
        if not os.path.exists(targetFilePath2):
            os.makedirs(targetFilePath2)

        for studentId in alarmSet:
            cStudentAlarm = alarmSet[studentId]
            #print("LC--总学分：{0}，姓名：{1}".format(cStudentAlarm.gettotalCredit(), cStudentAlarm.getstudentName()))
            if(cStudentAlarm.gettotalCredit() >= 10.0 and cStudentAlarm.gettotalCredit() < 20.0):
                fileName = targetFilePath + "\\" + cStudentAlarm.getclassName() + "班_" + cStudentAlarm.getstudentName() + "_学业预警.docx"
                earlyAlarm(cStudentAlarm, fileName)
            elif(cStudentAlarm.gettotalCredit() >= 20.0 and cStudentAlarm.gettotalCredit() < 25.0):
                fileName = targetFilePath2 + "\\" + cStudentAlarm.getclassName() + "班_" + cStudentAlarm.getstudentName() + "_学业警告.docx"
                currentAlarm(cStudentAlarm, fileName)
        return True
    except BaseException as err:
        print("{0}".format(err))
        return False

def loopDel25Credit(alarmSet, deletePath):
    try:
        for curNotPassInfo in alarmSet.getnotPassInfo():
            fileName = deletePath + "\\" + alarmSet.getclassName() + "班_" + alarmSet.getstudentName() + "_" + curNotPassInfo.getcourseName() + ".docx"
            print("删除 - {0}".format(fileName))
...
def delete25Credit(alarmSet, targetPath):
    try:
        targetFilePath = targetPath + "重修登记表"
        if not os.path.exists(targetFilePath):
            return

        for studentId in alarmSet:
            cStudentAlarm = alarmSet[studentId]
            if(cStudentAlarm.gettotalCredit() >= 25.0):
                print("删除总学分：{0}，姓名：{1}的登记表".format(cStudentAlarm.gettotalCredit(), cStudentAlarm.getstudentName()))
                loopDel25Credit(cStudentAlarm, targetFilePath)
        return
    except BaseException as err:
        print("{0}".format(err))
        return
...
def retakeDealExl(firstFilePathAndName='系统导出未通过', secondFilePathAndName='教师任务表一', thirdFilePathAndName='教师任务表二'):
    ...
    try:
        ...
        resultDir = path + "重修登记表"
        if os.path.exists(resultDir):
            shutil.rmtree(resultDir)

        resultDir = path + "学业预警通知单"
        if os.path.exists(resultDir):
            shutil.rmtree(resultDir)
        ...
        currentNotPassInfo = notPassInfo()
        alarmNotifyPeopleSet = {}
        ...
        while (notPassSheetIndex < notPassSheetsNum):
            ...
            while (rowIndex < rowNum):
                ...
                if(63 == oneRemakeNotify.gethaveDataFlag()):
                    ...
                    if(oneRemakeNotify.getstudentId() in alarmNotifyPeopleSet):
                        currentAlarmInfo = alarmNotifyPeopleSet[oneRemakeNotify.getstudentId()]
                    else:
                        currentAlarmInfo = alarmNotifyInfo()
                        currentAlarmInfo._init_()
                        currentAlarmInfo.setclassName(oneRemakeNotify.getclassName())
                        currentAlarmInfo.setstudentName(oneRemakeNotify.getstudentName())
                        currentAlarmInfo.setstudentId(oneRemakeNotify.getstudentId())

                    currentAlarmInfo.addtotalCredit(oneRemakeNotify.getcredit())

                    currentNotPassInfo.setsemester(oneRemakeNotify.getsemester())
                    currentNotPassInfo.setcourseName(oneRemakeNotify.getcourseName())
                    currentNotPassInfo.setachievement(oneRemakeNotify.getachievement())
                    currentNotPassInfo.setcredit(oneRemakeNotify.getcredit())
                    currentAlarmInfo.addnotPassInfo(currentNotPassInfo)
                    currentNotPassInfo.cleanData()
                    #print("LC--{0}".format(currentAlarmInfo.getnotPassInfo()))

                    alarmNotifyPeopleSet[oneRemakeNotify.getstudentId()] = currentAlarmInfo
        ...
        delete25Credit(alarmNotifyPeopleSet, path)

        if (False == generateAlarm(alarmNotifyPeopleSet, path)):
            print("学业预警通知生成失败失败\n")
        return
    except BaseException as err:
        print("{0}".format(err))
...
def generateNotification(notifyInfo, targetPath):
    try:
        targetFilePath = targetPath + "重修登记表"
        fileName = notifyInfo.getclassName() + "班_" + notifyInfo.getstudentName() + "_" + notifyInfo.getcourseName()
        if not os.path.exists(targetFilePath):
            os.makedirs(targetFilePath)
        ...
        tabP = table.cell(1, 3).paragraphs[0]
        if(notifyInfo.getspecial() == '缺考'):
            tableContent = tabP.add_run(str(notifyInfo.getspecial()))
        else:
        ...
        value = str(15 * notifyInfo.getcredit()) + '/' + str(notifyInfo.getcredit())
...
输入 'q' or 'Q' 退到上一层\n")
            ...
            if (retakeInpute == "d" or "D" == retakeInpute):
                ...
                retakeFirstExelName = input("输入重修系统导出未通过表格路径加文件名，或把文件放到和工具同名目录下直接输入文件名，不带后缀。\n")
                #retakeSecondExelName = input("输入教师任务表一路径加文件名，不带后缀。\n")
                #retakeThirdExelName = input("输入教师任务表二路径加文件名，不带后缀。\n")
                retakeDealExl(retakeFirstExelName)
